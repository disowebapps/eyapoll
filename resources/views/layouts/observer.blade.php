...
<head>
    ...
    <title>{{ config('ayapoll.platform_name', 'Echara Youths') }} - @yield('title', 'Observer Dashboard')</title>
...
<body class="bg-slate-50" x-data="{ sidebarOpen: false }">
    <!-- Sidebar Component -->
    <x-sidebar type="observer" :user="auth('observer')->user()" />
        ...
        <header class="sticky top-0 z-40 bg-white border-b">
            <div class="flex items-center justify-between h-16 px-6">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">@yield('page-title', 'Observer Dashboard')</h1>
                </div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" class="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50">
                            <img src="{{ auth('observer')->user()->profile_image_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth('observer')->user()->full_name) . '&size=32&background=7c3aed&color=ffffff' }}" 
                                 alt="Profile" 
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border py-1 z-50">
                            ...
                            <a href="{{ route('observer.profile') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
