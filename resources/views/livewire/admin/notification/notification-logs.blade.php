<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">Monitor and track all notification deliveries</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div>
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text"
                           ...
                           placeholder="Email, phone, message...">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Channel</label>
                    <select wire:model.live="channelFilter" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                ...
                <div>
                    ...
                    <select wire:model.live="statusFilter" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        @if($log->channel === 'email') bg-blue-100 text-blue-800
                                        @elseif($log->channel === 'sms') bg-green-100 text-green-800
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    @if($log->recipient_email)
                                        {{ $log->recipient_email }}
                                    @elseif($log->recipient_phone)
                                        {{ $log->recipient_phone }}
                                    @else
                                        User #{{ $log->recipient_id }}
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $log->notification->type ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        @if($log->status === 'sent') bg-green-100 text-green-800
                                        @elseif($log->status === 'failed') bg-red-100 text-red-800
                                        @elseif($log->status === 'delivered') bg-blue-100 text-blue-800
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">
                                    ...
                                    @if($log->error_message)
                                        <div class="text-red-600 text-xs mt-1 truncate" title="{{ $log->error_message }}">
                                            Error: {{ Str::limit($log->error_message, 30) }}
                                        </div>
                                    @endif
                                </td>
                            </tr>
